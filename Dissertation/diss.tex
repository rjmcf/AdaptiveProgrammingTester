...
The aim of the \texttt{ExerciseSetter} class is to be capable of delivering programming exercises, measuring the performance of students who solve these exercises, and adapting the difficulty of the following exercises appropriately. It has been shown in Section \ref{Sec:ExSet} that the \texttt{ExerciseSetter} class can deliver exercises by printing the question and a \texttt{String} representation of the incomplete model solution. In Section \ref{Sec:Blanks} the algorithm that adapted the difficulty of exercises was shown. All that remains is to analyse the way performance is measured.

The class has a method \texttt{reportPerformance} which returns a \texttt{float} value that represents the performance of a student once they have solved an exercise. If this value is positive, the student has performed to the required standard to be given a more difficult exercise, whereas if it is negative then the next exercise will be easier. Figure \ref{Fig:PerfGraph} shows how the measured performance depends on both the number of attempts a student has made to give a correct solution and also the number of extra nodes they used in their solution compared with that of the model solution. It shows the contours of the surface, which is a function of two variables. The orange line labelled 0 represents the zero contour, such that any solution that maps to a point above that line (on the 2D plot) is unsatisfactory, and will result in the next exercise being easier. Solutions mapping to points below the zero contour are of a standard such that the next exercise will be harder.

The performance varies as a function of the number of attempts squared, punishing solutions that required multiple attempts, as this indicates a lack of understanding of the question being answered. The cut-off point for the number of attempts is 2.5, that is if a student requires two attempts or fewer, the system determines that the next exercise can be harder, but if a student requires three attempts or more, the next exercise will be easier. This was chosen to allow for one careless mistake before assuming lack of comprehension.

If the student uses fewer nodes than the model solution, then either they have found a more efficient solution, or they have ``cheated'' and not used the appropriate syntax constructs being taught (i.e. reached $n!$ by simply calculating $n\times(n-1)\times\dotsb$ all at once rather than using the expected \texttt{while} loop). At present, when a student uses fewer nodes, the system cannot tell the difference between a student showing greater understanding and one ``cheating''. Therefore, the performance function does not take this into account, shown by the horizontal contours to the left of the $x=0$ vertical. When the number of extra nodes used is greater than zero, it is almost certainly a sign that a student has not understood the best way to solve the question (i.e. unrolled the while loop to calculate $n!$). For this reason, the greater the number of extra nodes used, the worse the measured performance.

An area of future work might be to enable the system to check for certain code constructs being used in the presented solution. This would enable differentiation between students using constructs more efficiently, and those simply not using them.

\begin{figure}
  \centering
    \includegraphics[width=0.8\textwidth]{PerfGraph}
  \caption{A graph showing the contours of the performance surface, dependent on the number of attempts required and the number of extra nodes used.}
  \label{Fig:PerfGraph}
\end{figure}
...
Since the GUI also makes use of the parser to allow students to directly edit the model solution however they choose, any use and evaluation of the GUI also functions as a test of the parser.
...
\centering
  ...
  \begin{subfigure}{.55\textwidth}
    ...
    \label{SFig:GGS3}
  \end{subfigure} 
\caption{Screenshots of the GUI when the student has performed to a standard deserving of a more difficult exercise.}
