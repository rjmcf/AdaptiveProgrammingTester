...
In this chapter I will detail the work completed. I will begin by describing how I implemented MiniJava, (a small imperative programming language based on Java), as well as a parser for MiniJava's grammar. I then describe the algorithm that allows the addition and removal of ``blanks'' within a MiniJava program. I will then describe how a programming exercise is conceptualised in the abstract sense, and give an example of an implementation of a specific question. Finally, I will describe the two peripherals I developed to interact with the language; the \texttt{ExerciseSetter} and the Graphical User Interface (GUI).

\section{The Language: MiniJava}

In this section I will describe how I implemented the MiniJava language.
...
MiniJava is based on a subset of the Java 8 specification (\cite{Java8}  page 714) formed by removing some of the more complex language features. Importantly, only single-dimensional arrays are allowed. A full description of the parser for the language can be found in Appendix \ref{App:Parser}. MiniJava represents language features as classes, for example, there is a class called \texttt{CondExpr} that represents a conditional expression. Fig \ref{Fig:UML} shows the top levels of classification that all language features implemented by MiniJava fall into. Ignoring \texttt{MiniJASTNode} for a moment, all language features can be categorised as being either \texttt{Expression} or \texttt{BlockStatements}. Every \texttt{Expression} extends the abstract class \texttt{ExpressionBase}. Some expressions, like \texttt{AssignExpr}, also implement \texttt{StatementExpression}, while others, like \texttt{CondExpr}, do not. All other expressions extend \texttt{CondExpr}, which gives rise to an implicit operator precedence. All \texttt{BlockStatements} subclass \texttt{StatementBase}. A MiniJava program can be represented as a tree similar to a parse tree. Thus every class involved needs to implement \texttt{MiniJASTNode} so that operations can be performed on every node of the tree. 

Looking at the \texttt{Expression} interface, we see the two methods \texttt{evaluate} and \texttt{stringRepr}. The method \texttt{stringRepr} simply returns a \texttt{String} representation of the \texttt{Expression}, evaluated recursively through its sub-nodes. For example: 
...
In this section I will explain how a parser for MiniJava was implemented using the ANTLR parser generator \cite{ANTLR4}, and how a MiniJava program tree can be built using it.

The GitHub user antlr that owns the antlr4 project containing ANTLR has another project called grammars-v4 \textcolor{red}{[Reference to  https://github.com/antlr/grammars-v4]} that contains lots of example grammars that can be used with ANTLR, including one for Java. I was able to modify this grammar file, in a similar way to how I modified the Java specification when designing MiniJava, to make a parser generator for MiniJava, which can be found in Appendix \ref{App:Parser}. When you use antlr4 on the grammar file, the tool generates a number of files including a \texttt{<GrammarName>BaseVisitor}. By extending this class, one can add actions to the parser. By overriding the appropriate methods in this class, I was able to implement a parser that, given valid MiniJava code, could build a MiniJava program tree.

\section{The Abstract Question}

In this section I will explain the formulation of the abstract question and give an example of how a concrete question can be derived from it.
...
An example implementation of these abstract methods is found in \texttt{FactorialExercise}, an exercise in which the student is asked to use a while loop to calculate the factorial of some integer \texttt{n}, which is supplied on construction of the object. The overrided \texttt{setUp} method constructs the model solution, which represents the code:
...
\begin{description}
\item[This node's children has already been searched:] {In this case, we need to check whether all of this node's children are blank. If they are, then technically this node is a leaf, but we don't want to replace it next walk of the tree so we mark it accordingly. Either way, we move on to the next node.}
...
Within the \texttt{while(true)} loop the entire solution tree is walked. If a result has not been reached in one iteration of the loop, then all the leaves are marked for not being selected this walk of the tree, so we change the marking we are looking for and walk the solution tree again.
...
In this section I will describe how I made use of the language and its features to design the \texttt{ExerciseSetter}, one possible way in which students might interface with exercises designed to facilitate the learning of MiniJava.
...
In this section I will describe the implementation and function of the GUI peripheral.
