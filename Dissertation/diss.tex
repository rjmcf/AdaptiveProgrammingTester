...
\begin{center}
\Huge
\textbf{A System For Giving Programming Exercises Adaptive Difficulty} \\[5mm]
...
Project Title:      & \bf A System For Giving Programming Exercises \\ & \bf Adaptive Difficulty \\
Examination:        & \bf Computer Science Tripos -- Part II, July 2017  \\
Word Count:         & \bf 11,826  \\
...
The original aims were to design a system that measures the performance of a student as they solve programming exercises and algorithmically determines the difficulty of the next exercise based on that performance. This required implementing a language which incorporated ``blanks'' into its grammar, writing an interpreter for this language, encoding representations of programming exercises, developing algorithms to add and remove blanks from programs, and writing heuristics to measure performance and adjust question difficulty. Implementing a parser and a GUI were possible extensions, as was measuring competence with different syntax constructs separately (e.g. \texttt{for} loops and \texttt{if} statements), and producing graphs of progress against time to motivate students.

\section*{Work Completed}

I was able to fulfil all the core aims of the project and implement two extensions. I managed to implement a new language, write an interpreter for it, encode representations of programming exercises, implement algorithms to add and remove blanks from programs, and write heuristics to measure performance and adjust question difficulty. I also implemented a parser and designed and implemented a GUI as my two extensions.
...
I would like to thank my Project Supervisor, Michael Gale, and my Director of Studies, Dr John Fawcett, for all their support and guidance. Thanks also to my whole family and all my friends, for reminding me that I am never alone. This would not have been possible without you.
...
Finding the most effective way to teach young people new skills is an important challenge, impacting educators and learners. Students want to be given information in an interesting and engaging way that enables them to make use of it constructively, and educators want to be effective. How well students learn a new skill is determined by a number of factors, ranging from differences between learners (e.g. ability and motivation) to the teaching method used. It is debatable whether unguided or guided learning styles produce better results \cite{GuidedVsUnGuided}, and similarly tasks requiring problem solving have been compared with those requiring reciting known solutions \cite{PBL}.

Recently, Computer Science has been included into the UK national curriculum for children aged 5 and above. This has raised the question of how best to teach young people a programming language (e.g. Java). The typical method employed in the classroom is to give instruction (typically including worked examples) followed by programming exercises.
 
Exercises can be conceptualised as a question and a sample of incomplete code. A student must fill in the blanks such that the completed code can perform the function required by the question. When students have a firm grasp of the language, it may be reasonable to expect them to write code without guidance. However, when a student is first learning a language, is less able, or younger, they may find it helpful to have parts of the code provided. This enables them to see how the syntax is used, reduces the complexity of the work, and is likely to enhance the student's learning experience and maintain their motivation to continue. 

\section{Context of the work}

A previous attempt to vary exercise difficulty in response to student performance \cite{PhDThesis} used computerised-adaptive testing (CAT) by developing a system that enables students to manage their own user profile, which determines the difficulty of their exercises. Students also assess their own performance which provides a measure of their understanding of the subject matter. The system was tested and found increased mean scores in students who used it versus students who did not. This shows that CAT systems have promise.

\section{Project aims}

A different approach to CAT would be to measure student performance algorithmically, inferring how much the student has understood and presenting students with problems varying in complexity as a function of their understanding of the material. The specific questions that the students have to answer would be determined by their teacher and the appropriate curriculum. However, the amount of code that each student is expected to write can be adjusted algorithmically, by adding or removing blanks from a model solution.

This project aims to develop a system that utilises this approach to teach a programming language by responsively setting exercises for students. The first problem presented will always be the same, and each student's performance will be assessed by the system. The difficulty of the next problem will be adjusted appropriately by changing the amount of the model solution that is left blank to fill in. If it is determined that the exercise is too easy or too hard, given the current competence of the student, then the exercise itself will be changed.

To develop this system, I design a new language that can incorporate gaps in programs and write an interpreter and pretty-printer for it. After this, a representation of programming exercises is encoded, and an algorithm that adds and removes blanks from programs is written. These tools are brought together into a system which measures the performance of a student using a heuristic and adjusts the difficulty of programming exercises appropriately.

Extensions to this work include writing a parser for the new language, and implementing a Graphical User Interface (GUI). This GUI demonstrates how a student may interact with the system, by presenting model solution code that students can directly edit to fill in the blanks, and giving feedback on solutions.

The aims of this project are to:
\begin{enumerate}
\item{Develop a new language, incorporating the concept of blanks in programs.}
\item{Implement an interpreter and pretty-printer for this language.}
\item{Encode a representation of programming exercises.}
\item{Write algorithms that add and remove blanks from programs.}
...
In this chapter, I detail the preparation I undertook before starting my implementation. I explain what experience I had before this project began in Section \ref{Sec:SP}. In Section \ref{Sec:Overview}, I give a brief overview of the project and how its components fit together. Then, in Section \ref{Sec:ReqAnal} I give a detailed requirements analysis, explaining my choices of methods and libraries. In Section \ref{Sec:Method}, I explain the specific software engineering methodologies I employed to ensure implementation went as smoothly as possible and finally, in Section \ref{Sec:SRS}, I give the concrete deliverables for my project as a Software Requirements Specification.

\section{Starting point}
\label{Sec:SP}

I have experience in programming in Java (the language in which I will be implementing my tools) and some experience of writing interpreters, primarily of Combinators. I designed an educational program as an Extended Project for the AQA Baccalaureate, the aim of which was to investigate if using a computer could make Key Stage 2 students more motivated to learn maths. This involved automating the maths tests in their textbooks by designing templates that were instantiated at run time to provide a variety of questions. I have experience designing GUIs, since I used one for my Extended Project, and another was an integral part of the Group Project last year. Finally, I have experience of teaching and working with children through volunteering as a teaching assistant with a Key Stage 2 class and co-directing the Junior school play, training as a tennis coach and working with Cambridge Coding Academy.

\section{An overview of the project and its components}
\label{Sec:Overview}

The overall aim of this project is to design a system that algorithmically adapts the difficulty of programming exercises. Decisions about how to adjust the difficulty are made by referring to measurements of student performance taken as they solve preceding exercises. In order to set programming exercises, we need a language to solve them with. Representations of programs written in this language can be pretty-printed, and are executable by interpreting the code. Programming exercises themselves require representations within the system, and these store the AST of their model solution. Wrapped around these exercises is a system for presenting questions, allowing completion of the model solutions, checking validity of submissions, and delivering feedback. This system incorporates algorithms that increase and decrease the difficulty of problems by adding and removing blanks from the model solution code. As an extension, a parser is implemented to streamline some functionality: the model solution in exercises is defined in a \texttt{String} that is then parsed to build the AST representation required, and the system fills blanks by parsing \texttt{String}s containing the code to be substituted. Finally, as another extension, a GUI is implemented that acts as an interface between the system, students and teachers, making use of the parser to allow students to directly edit the model solution code.
...
Ideally, I would have preferred to simply add the concepts of blanks to Java itself. However, investigation showed that this was more problematic than anticipated.

An early prototype I built to investigate how to add blanks to Java involved storing a program as a \texttt{String} and executing it as Java code. I have done this in Python, where dynamic execution of statements and expressions is supported by the \texttt{exec} and \texttt{eval} functions respectively. Unfortunately, doing this in Java is much more complex, requiring the use of external tools which have no inbuilt support for blanks in code. I would have had to write some substantial extensions to these tools, and therefore decided to follow an alternate path.

I decided to design my own language that would very closely imitate Java, but include the concept of blanks in code within the grammar itself. This language would imitate Java in order to demonstrate that Java itself, a language taught to many beginners, could be taught using my system. For the purposes of this project it was unnecessary to re-implement all of Java. I thus designed my language, which I call MiniJava, to imitate a small subset of the Java grammar \cite[p.714]{Java8}. 

\subsection{Interpreting and pretty-printing the language}

Representations of programs written in MiniJava should be executable and be able to be pretty-printed. As an extension, it would be nice to take a textual version of a program and convert it into an AST representation by parsing it. 

An interpreter of MiniJava must allow the declaration of new variables, manage their scope, and update them when they are assigned. The most effective methods for managing variables as the interpreter runs are to store them in symbol tables or in stack frames. Early prototypes showed that storing variables in symbol tables made the management of scope more complex, as the ``depth'' at which a variable is declared must be explicitly stored too, so that the variable can go out of scope at the appropriate time. When variables are stored in stack frames, scope management becomes implicit within the structure of the stack, so I decided to use this method. 

\subsection{Encoding representations of programming exercises}

In my system, I define a programming ``exercise'' as consisting of a question, represented by a \texttt{String}, and a model solution, represented by a MiniJava AST. An exercise allows the blanks in its solution to be filled in by students, and complete solutions are executable. Exercises record constraints that must be satisfied for a solution to be declared valid (e.g. if the question is solved by setting the variable \texttt{result} to a certain value, then the solution is validated by checking that the variable has that value once the solution is run). They are made harder or easier by adding or removing blanks from their model solution. The algorithm I designed to do this is described in Section \ref{Sec:Blanks}, and takes inspiration from both the standard Depth First Search algorithm and the more obscure Reverse Level Order Traversal. Designing this algorithm was a significant challenge during the preparation phase of this project.
...
An exercise is set by presenting first its question, and then the incomplete model solution to the student. The blanks in this solution should be conspicuous and they should be editable. If students can not directly modify the code, this will require labelling each blank with a unique identifier, so that students can select which one to fill. Once the solution has been submitted, the solution must be run so that the system can decide whether it solves the problem. The system will interpret the code, since there are many possible correct solutions. Feedback needs to be delivered to the student, telling them that their solution was correct or that they have made a mistake. 

Once a solution has been validated, the performance of the student must be measured. Performance can be measured in many ways but for the purposes of this project I have defined the performance heuristic as dependent on the number of attempts a student required in order to produce a correct solution, and the number of nodes in their solution's AST compared with the model solution's AST. If a student required multiple attempts to solve the question it seems likely that they have not understood the underlying concepts being tested. Similarly, using many more nodes than is necessary may indicate a lack of understanding, and thus results in a lower performance measurement. 

Once the system has measured how the student performed, it decides how difficult their next exercise should be to best match their competence. For example, if it is determined that a student has mastered the current question, then they will be presented with a more difficult question. However, if they have performed well, but not fully understood the material, then the next question will increase the amount of the model solution that is left blank. 

\subsection{Extension: Parsing the language}

Writing a parser of the language is an extension to this project, as a parser is not necessary to demonstrate the power of the system. I decided that if I did implement a parser I would use a parser generator, as writing a parser myself would be time consuming and error prone. Of the available parser generators, I investigated ANTLR \cite{ANTLR4} and JavaCC\footnote{https://javacc.org/ - last accessed 3rd May, 2017}, as both are popular and use Java. Both support tree building (necessary for building the AST representation of a program in my implementation of the language), however JavaCC provides this as a separate tool while ANTLR provides the functionality by default. Looking at how the two tools are used to generate parsers for very simple grammars, I found ANTLR's work flow much more amenable. ANTLR also has more high profile users, like IntelliJ IDEA, Groovy\footnote{http://groovy-lang.org/ - last accessed 18th May 2017}, Twitter and Numbers (Apple's spreadsheet program). Based on this, and my own opinion that it is more intuitive to use, I decided to use ANTLR if I implemented this extension.

\subsection{Extension: Implementing a GUI}

The GUI should act as an interface between the system, teachers and students. Teachers should be able to manually adjust the difficulty of a given question. Students should be shown the question alongside the code representation of the incomplete model solution. If a parser has been implemented, the GUI should allow direct editing of the model solution, the result of which can then be parsed and set as the solution to the exercise to allow validation. The most popular frameworks for implementing GUIs in Java are Swing\footnote{https://docs.oracle.com/javase/7/docs/api/javax/swing/package-summary.html} and JavaFX\footnote{http://www.oracle.com/technetwork/java/javafx/documentation/reference-137773.html}, since they have the most support. Although I have more experience working with JavaFX from the group project, the Swing plugin for IntelliJ (the IDE that I am using) has a drag and drop interface for designing UIs, and greatly improves the experience of working with Swing, so I decided to use this if I implemented the extension.

\section{Methodologies in use}
\label{Sec:Method}

In order to ensure correct functionality of my system I used Test Driven Development (TDD). This required writing unit tests for code that had not yet been written. These tests were designed with respect to the requirements, such that any component passing the tests also satisfies the requirements. These tests were then kept to act as regression tests.

\section{Software Requirements Specification}
\label{Sec:SRS}

The following requirements are annotated with the appropriate identifiers from the MoSCoW method for prioritising requirements\footnote{http://research.omicsgroup.org/index.php/MoSCoW - last accessed 10th May}.

\begin{enumerate}
\item{A language supporting blanks will be implemented. \textbf{M}}
...
\item{Exercises will have parts that are randomly generated to provide variety. \textbf{N}}
\item{A system will be developed to measure the performance of a student and adjust the difficulty of exercises. \textbf{M}}
...
Each class in my implementation represents a different production in the abstract syntax for MiniJava, found in Appendix \ref{App:Parser} in the form of a parser specification. In order to allow other parts of my code to operate on instances of these classes without needing to know exactly what they are, every class in the implementation implements the \texttt{MiniJASTNode} interface. This acts as the top of the class hierarchy for my implementation, as shown in Figure \ref{Fig:TopLevelUML}.		
 ...
 \caption{The UML diagram showing how blanks fit into the class hierarchy, motivating the need for both abstract classes and interfaces to distinguish between expressions and statements. Other classes such as \texttt{ExpressionsBase} and \texttt{StatementBase} are excluded for clarity.}		
 \label{Fig:BlanksUML}		
 \end{figure}		
 		
 Since classes in Java cannot extend more than one base class, the only alternative is for them to implement the \texttt{Expression} and \texttt{Statement} interfaces. However, removing \texttt{ExpressionBase} and \texttt{StatementBase} would result in too much repeated code across the whole grammar, so both the interfaces and the abstract classes are required.		

As important as the distinction between expressions and statements is, there do exist some expressions that can be used like statements. For an example, the MiniJava code \texttt{i~=~5;} is a statement (indicated by the semicolon,~``\texttt{;}''), but this statement consists entirely of the assignment expression \texttt{i~=~5}\footnote{Considering assignments as expressions allows for the chaining of assignments found in the Java grammar \cite[p.589]{Java8}.}. This sort of expression within a statement construct is not possible with all expressions, for example the code \texttt{i~+~4;} would not be valid MiniJava. To make the distinction between expressions which may be used in statements like this and those which may not, I made the interface \texttt{StatementExpression}, again, shown in Figure \ref{Fig:TopLevelUML}. This interface is only implemented by those expressions which can be used in a statement this way.

\subsection{Implementing operator precedence and associativity}

A feature of MiniJava is the concept of operator precedence, which is used to disambiguate the grammar of expressions. Without the concept of operator precedence, the code \texttt{1~+~2~*~3} is semantically ambiguous, as it can be read as either \texttt{(1~+~2)~*~3} which equates to 9, or as \texttt{1~+~(2~*~3)} which equates to 7. Since a single arithmetic expression cannot have more than one value, a decision must be made as to which of these interpretations is declared valid. The decision taken aligns with the standard arithmetic order of operations, such that the answer of 7 is the correct answer in this case. This example demonstrates that the multiplication operator, ``\texttt{*}'', has a higher precedence than the addition operator, ``\texttt{+}''. All the possible operators in MiniJava are found within this precedence hierarchy, and I needed some way to encode this. The method I chose for this is the natural choice, since it is suggested by the Java grammar itself \cite[p.723]{Java8}. The production for \texttt{AdditiveExpression} is as follows:
...
\end{Verbatim}
and the production for \texttt{MultiplicativeExpression} is as follows:
\begin{Verbatim}[samepage=true]
...
\end{Verbatim}
This means that for any expression $A$\texttt{ + }$B$, the expression $A$ must only contain subexpressions that use operators with precedence at least as high as \texttt{+} and \texttt{-}, and $B$ must only contain subexpressions that use operators with precedence higher than \texttt{+} and \texttt{-}. For any expression $A$\texttt{ * }$B$, neither $A$ nor $B$ can contain subexpressions that use operators with precedence lower than \texttt{*} and \texttt{/}. This in turn indicates that an expression such as \texttt{1~+~2~*~3~+~4} can only be interpreted as \texttt{1~+~(2~*~3)~+~4}, and an expression such as \texttt{1~*~2~+~3~*~4} can only be interpreted as \texttt{(1~*~2)~+~(3~*~4)}. I wanted an operator precedence for MiniJava that is hardcoded within the grammar of MiniJava itself. Normally, these ambiguities of the grammar would be disambiguated by the parser, but since I would not necessarily implement a parser, I decided to have the implementation of the grammar itself prevent any ambiguities. The productions for Java shown above suggest a natural implementation of the class that represents addition expressions:
\begin{Verbatim}[samepage=true]
class AddExpr {
    ...
...
\end{Verbatim}
Here, \texttt{leftSide} and \texttt{rightSide} store the left and right operands of the expression, and \texttt{MultExpr} is the class representing multiplication expressions. What has been implemented so far encapsulates parts (2) and (3) of the \texttt{AdditiveExpression} production above, but not part (1). This part is encapsulated by making the class \texttt{MultExpr} extend the class \texttt{AddExpr}, such that an instance of \texttt{MultExpr} may always be used in place of an instance of \texttt{AddExpr}. Note that multiplication operators are higher in the operator precedence hierarchy and their representations are lower in the class hierarchy. This then is the solution to the operator precedence problem: to enforce an operator precedence hierarchy, first enforce an inverse class hierarchy in their representative classes, and then choose the type of each expression's operands to match the relevant terminal symbols in the relevant production.\footnote{Note that if you were to look at my implementation of \texttt{AddExpr}, \texttt{leftSide} and \texttt{rightSide} would both be \texttt{int}s, not \texttt{AddExpr} and \texttt{MultExpr} instances. The reason behind this is explained in section \ref{Sec:Blanks}.}

Note also that as a side-effect of this implementation, the syntactically ambiguous code \texttt{1~+~2~+~3} also only has one possible interpretation, namely \texttt{(1~+~2)~+~3}. This follows the established rules of operator associativity, which are now also hardcoded into the grammar.

\subsection{Expression types}

In MiniJava, every expression has a primitive type and may or may not be an array. There are four possible primitive types: \texttt{boolean}, \texttt{char}, \texttt{int}, and \texttt{double}. The primitive types are represented in the grammar by the enumeration, \texttt{PrimType}. The actual type of an expression is represented by the class \texttt{Type}, an instance of which stores the appropriate \texttt{PrimType} value and a flag determining whether or not the expression is an array. These \texttt{Type}s are used by the interpreter to determine whether or not variables can contain certain values, and whether operators are being supplied with the appropriate operands.

\subsection{Blanks in code}

What distinguishes MiniJava from other languages is that the concept of a blank, a missing piece of code, is built into the grammar itself. Representations of blanks in code should enable a user to fill one with their own code. To do this algorithmically in the absence of a parser, it is necessary for representations of blanks to each come with a unique id. This means that users can choose which blank they are filling, and the system can search for blanks within the AST representing the program. The abstract class \texttt{FillableBlank} implements this functionality. Concrete classes extending this class are \texttt{FillableBlankExpr} (which represents a blank to be filled with an expression) and \texttt{FillableBlankStmnt} (which represents a blank to be filled with a statement). Both are required because the interpreter treats expressions and statements differently. 

If a parser for the language is implemented then the functionality of blanks having unique identifiers becomes obsolete, since we can allow the student to directly edit the model solution and simply parse the result. However, even if a parser has been implemented, there may be applications where you do not want a student to edit all of the code, in which case you would still need to have a way to select blanks, and identifiers would be required here.

I explain how blanks are inserted in to and removed from programs in Section \ref{Sec:Blanks}.
...
\begin{enumerate}
\item{The scope of variables must be handled, such that when a variable is used its current value can be accessed, and the same variable can not be declared twice in one scope.}
...
If students can edit all the code and have their solutions parsed, then blanks are not so much filled as they are eliminated. However, in the case that students are not allowed to edit all the code we need some way to fill blanks programmatically.

Since representations of blanks in code have their own unique identifier, the blank that a user wishes to fill can be selected using its identifier. To fill a blank with the representation of a user's code, the representation of the blank itself must be found within the program's AST, before the representation of the code is stored within it. Since the tree is not structured, that is the nodes are not sorted, searching the tree is $O(n)$ in the number of nodes. Since blanks are more likely to be found at the bottom of the tree than the top (see Section \ref{Sec:Blanks} for justification), this is done using a depth-first-search through the AST to find blanks which, when found, have their identifiers compared with the target identifier. If they do not match, then the search moves on. If they do match, then the blank is filled with the student's code. If the search fails, then it can be reported that no blank with the suggested identifier exists within the solution.

\subsubsection*{3. Checking that the problem is solved by the provided solution}

Given that the amount of information about a program's execution that can be attained after a program has finished execution is limited, there are two possible ways of checking whether a solution is valid. Either the value of a variable can be compared with a target value, or the result of a print statement can be inspected. The first of these approaches requires an \texttt{Id} field to be declared in the concrete class representing the problem. Once the solution has been executed, the final value of this variable can be accessed by evaluating the \texttt{Id} with the \texttt{Context} object used during execution. In contrast, the second approach can be implemented by checking the contents of the file written to by the representation of the print statement. 
...
In contrast to adding a blank, the algorithm that removes a blank from the model solution is much simpler. We simply take the top off the stack of replaced nodes and the stack of the locations of replaced nodes, navigate to the correct location and reinsert the node back in to its parent.
...
The parser grammar (which can be found in Appendix \ref{App:Parser}) is based on an existing ANTLR parser grammar for Java\footnote{https://github.com/antlr/grammars-v4/tree/master/java - last accessed May 13th 2017}. It seemed unnecessary to write an entire grammar from scratch when there was such a readily available alternative. It is always good to use existing libraries or tools if they are available. 
...
The following code shows how to build a MiniJava representation for the source code \texttt{i~=~4;} without a parser.
...
In this chapter I demonstrate that all of the original goals of the project were achieved. I show in Section \ref{Sec:Func} that all the components fulfil the requirements laid out in Chapter 1. The trends that emerge as the difficulty is adjusted are analysed in Section \ref{Sec:DiffTrends}, and the development process is critiqued in Section \ref{Sec:DevProc}.
...
I explained the way that the \texttt{ExerciseSetter} class measured the performance of a student by reference to their correct solution in Section \ref{Sec:ExSetEval}. In this section I explain how the performance measurement is used to determine the difficulty of the subsequent exercise, and the trends that arise from this method.

\begin{figure}
\centering
\includegraphics[width = .9\linewidth]{DiffGraph}
\caption{Increase in measured difficulty with repeated correct solutions each with a measured performance of one.}
\label{Fig:DiffGraph}
\end{figure}
...
\caption{Demonstrating the different rates of increase of difficulty found for the different questions. The straight lines are extrapolated past the original points to show that they intersect and thus are not parallel.}
...
In conclusion the project was a success, as the software I developed fulfils all of my original aims. Furthermore, I was able to implement two extensions on top of the core functionality that have enabled me to demonstrate the power of the underlying system.

I have designed a new language, MiniJava, such that its programs can incorporate blanks that must be filled in before they can be executed. I have written an interpreter and a pretty-printer for this language, to execute arbitrary code and print representations of programs as MiniJava code. I encoded a representation of programming exercises and wrote algorithms to introduce blanks to and remove them from the model solutions for these exercises, making them harder and easier respectively. Using concrete examples of such exercises I implemented a system that sets exercises for a student, allows them to fill in the blanks with their own arbitrary code, and executes the completed code to determine whether the problem was solved. This system measures the performance of the student using a heuristic and adjusts the difficulty of the next exercise based on this measurement. As extensions, I implemented a parser for MiniJava to streamline the process of accepting student input, and created a GUI to demonstrate how both teachers and students might use the system. 

This project has several limitations. The choices I made regarding pedagogical practice were based on my experience as a learner and teaching assistant in a variety of settings. With hindsight it would have been better to use more theory to inform these choices. A further limitation is that the system has a limited number of questions that may be set which means that the system needs to present the same question multiple times. This means that currently it is possible for students to simply learn the answers through repetition of the material without actually understanding the underlying principles. 

\section{Further work}

MiniJava is only a subset of the full Java grammar. Since it is Java that students need to learn, adding the rest of Java's features to MiniJava would be worthwhile to enable the system to be used at a higher level. Currently this system would be useful for students who are beginning to learn Java as they only need to understand the features present in MiniJava, but it would not be useful for a student who needs to learn more complex principles such as how to write functions and classes.
...
In summary, this project has provided both a useful insight into the development of complex software in general, and also a successful implementation of a system for giving programming exercises adaptive difficulty.
...
\chapter{Pseudocode for \texttt{addBlank}}
\label{App:AddBlank}
\begin{Verbatim}[samepage=true,tabsize=4]
	...
	while (true) {
		...
		if (we find a leaf) {
			...
			if (the blank is the root of the tree)
				return false; // We can not add a blank to a blank solution
			else 
